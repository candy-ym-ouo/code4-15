...
- 来料质检、检验样本与缺陷登记、让步接收；质检通过后方可入库，驳回不产生批次。
- 批次入库、来源追溯、有效期和库存流水（批次只能由质检处置产生）。
- 库存调整、自动耗尽、归档和并发余额保护。
- 项目、材料需求、计划与实际消耗对比。
- 消耗、损耗、撤销和反向库存流水。
- 颜色变化时间线和批次当前颜色。
...
- 来料必须先报检：登记样本与缺陷后才能处置；合格接收/让步接收在同一事务内创建批次和首条 `OPENING` 流水。
- 质检处置只能落定一次：条件更新在数据库层占用质检单，并发处置只有一个事务生效，重复处置返回 `409`；`Idempotency-Key` 重试返回同一批次。
- 驳回的来料不产生批次、不产生库存流水；批次与质检单由数据库触发器双向绑定，一张质检单最多一个批次。
- 让步接收必须记录让步原因和批准人；存在严重缺陷（CRITICAL）时禁止让步接收。
