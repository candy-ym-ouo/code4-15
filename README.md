...
- 批次入库、来源追溯、有效期和库存流水。
- 来料质检：待检登记、检验样本、缺陷记录、合格接收/让步接收/驳回处置。
- 只有质检合格或让步接收才生成批次入库；驳回不产生批次；处置为终态，并发仅一次生效。
- 库存调整、自动耗尽、归档和并发余额保护。
- 项目、材料需求、计划与实际消耗对比。
- 消耗、损耗、撤销和反向库存流水。
- 颜色变化时间线和批次当前颜色。
...
验收脚本会真实创建来源、材料、批次、项目、需求、消耗和颜色变化，验证余额从 1000g 降到 500g、撤销后恢复 1000g、最新流水为 `REVERSAL`、颜色和搜索正确；还会验证来料质检链：待检不产生批次、并发处置仅一次生效、驳回不产生批次、让步接收仅让步数量入库、合格接收全部入库。
...
- 来料先登记为待检（PENDING），不生成批次、不写库存流水。
- 处置只有三种终态：合格接收（全部入库）、让步接收（让步数量入库）、驳回（不产生批次）。
- 批次和首条 `OPENING` 流水在处置事务内一起提交；驳回路径绝不创建批次。
- 处置是终态动作：行锁 + 状态判断 + “一检一处置”部分唯一索引，保证并发处置仅一次生效。
- 样本、缺陷只能在待检阶段登记；处置后检验单只读。
